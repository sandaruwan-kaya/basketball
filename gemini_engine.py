...
# This class defines the EXACT structure the model must return for consistency.
...
# -----------------------------------------------------
# Generic Gemini call (FIXED: Takes client as argument)
# -----------------------------------------------------
...
# -----------------------------------------------------
# Single Analysis Run Function (FIXED: Takes client as argument)
# -----------------------------------------------------
def _get_single_run_totals(client, prompt, video_path, chunks, fps):
    ...
    cumulative_totals = {
        "gemini-3-pro-preview": {"shots_attempted": 0, "shots_made": 0},
        "gemini-2.5-pro": {"shots_attempted": 0, "shots_made": 0},
    }
    ...
    for i, (start, end) in enumerate(chunks):
        ...
        if "error" not in g30_parsed:
            cumulative_totals["gemini-3-pro-preview"]["shots_attempted"] += g30_parsed.get("shots_attempted", 0)
            cumulative_totals["gemini-3-pro-preview"]["shots_made"] += g30_parsed.get("shots_made", 0)
        
        if "error" not in g25_parsed:
            cumulative_totals["gemini-2.5-pro"]["shots_attempted"] += g25_parsed.get("shots_attempted", 0)
            cumulative_totals["gemini-2.5-pro"]["shots_made"] += g25_parsed.get("shots_made", 0)

    return cumulative_totals
    
# -----------------------------------------------------
# Modified Main Analysis Function (FIXED: Passes client, Runs 5 times)
# -----------------------------------------------------
def gemini_analyse(prompt, video_bytes, log_dir, num_runs=5):
    ...
    # 2. Initialize Arrays for Final Output
    final_results = {
        "gemini-3-pro-preview": {"shots_attempted": [], "shots_made": []},
        "gemini-2.5-pro": {"shots_attempted": [], "shots_made": []},
    }
